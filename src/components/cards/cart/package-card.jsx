...
export default function PackageCard({ data }) {
  ...
  return (
    <div className='w-full border border-solid border-[rgba(255, 218, 50, 0.5)] shadow-[0px_-1px_6.9px_0px_rgba(0,0,0,0.25)] rounded-[20px] p-[25px] mt-[60px] flex gap-[20px]'>
      {data?.gallery &&
        <div className='h-[176px] w-[285px] rounded-[20px] overflow-hidden translate-y-[-48px]'>
        ...
        <div className='flex justify-between gap-2 w-full'>
          ...
          <div className='' >
            <div className='flex flex-col gap-5 items-end'>
              <div>
                <p className='text-[28px] leading-[42px] whitespace-nowrap'>{data.currency + " " + data?.price * count}</p>
              </div>
              <div className='flex flex-col items-center gap-3'>
                ...
                <p className='text-custom-red text-[12px] whitespace-nowrap'>Minimum 4 people required</p>
              </div>
            </div>
          </div>
        </div>
